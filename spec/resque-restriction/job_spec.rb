...
describe Resque::Job do
  ...
  it "should pop from restriction queue when reserve for unrestricted" do
  ...
  it "should leave on restriction queue when still restricted" do
  ...
  it "should only check queue_length times to restriction queue" do
    Resque.redis.set(OneHourRestrictionJob.redis_key(:per_hour), -1)
    3.times { Resque.push('restriction_normal', :class => 'OneHourRestrictionJob', :args => ['any args']) }
    Resque.size('restriction_normal').should == 3
    OneHourRestrictionJob.should_receive(:restricted?).exactly(3).times.and_return(true)
    Resque::Job.reserve('restriction_normal').should be_nil
  end
